...
# Find the block between 'let html = '';' and the end of renderInventory
# It looks like:
#     let html = '';
...
old_render_part = re.search(r"let html = '';\n\s*inventory\.sort\(\(a,b\).*?container\.innerHTML = html;\n\s*if\(typeof lucide !== 'undefined'\) lucide\.createIcons\(\);\n};", js, re.DOTALL).group(0)

new_render_part = """let html = '';
...
js = js.replace(old_render_part, new_render_part)

with open('/Applications/patuli_Stok_Takip/app_v4.js', 'w') as f:
    f.write(js)

print("Inventory rendering updated.")
