...
# We need to find the entire block of window.renderInventory
# It starts at "window.renderInventory = () => {"
# and ends right before "window.renderFinance = () => {" (if it exists) or just the end of the block.
# Actually, it's safer to just split it by finding the start, and counting braces.

start_idx = js.find('window.renderInventory = () => {')

if start_idx != -1:
    open_braces = 0
    end_idx = -1
    for i in range(start_idx + len('window.renderInventory = () => {'), len(js)):
        if js[i] == '{':
            open_braces += 1
        elif js[i] == '}':
            if open_braces == 0:
    ...
    new_func = """window.renderInventory = () => {
    ...
    with open('/Applications/patuli_Stok_Takip/app_v4.js', 'w') as f:
        f.write(js)
    print("Successfully replaced renderInventory.")
else:
    print("Could not find window.renderInventory.")
