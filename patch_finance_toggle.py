...
new_render_finance = """// Optional: Add global toggle function at the top of this block if we want it isolated, or just attach it to window
...
pattern = re.compile(r'window\.renderFinance\s*=\s*\(\)\s*=>\s*\{.*?(?=\Z)', re.DOTALL)
new_content = pattern.sub(new_render_finance, content)

with open('/Applications/patuli_Stok_Takip/app_v4.js', 'w') as f:
    f.write(new_content)

print("Patched renderFinance to add toggle behavior!")
